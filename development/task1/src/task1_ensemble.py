...
def run(save_path, agent_list, log_rules=False):
    ...
    gpu_id = int(sys.argv[1]) if len(sys.argv) > 1 else 0  # Use GPU 0 by default, pass -1 for CPU

    from erl_agent import AgentD3QN

    num_sims = 2**6  # Further reduced to 64 parallel environments for stability
    num_ignore_step = 60
    max_position = 1
    step_gap = 2
    slippage = 7e-7
    ...
    args.batch_size = 512
    args.break_step = int(16)  # Reduced for memory efficiency
    args.buffer_size = int(max_step * 8)  # Reduced buffer size
    args.repeat_times = 2
    args.horizon_len = int(max_step * 2)  # Reduced horizon length
