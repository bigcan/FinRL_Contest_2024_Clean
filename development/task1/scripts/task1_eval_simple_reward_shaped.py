...
from reward_shaped_simulator_simple import SimpleRewardShapedEvalTradeSimulator
...
class SimpleRewardShapedEnsembleEvaluator:
    ...
    def multi_trade(self):
        """Evaluation loop using ensemble of agents with simple reward shaping"""
        ...
        print(f"Starting evaluation with simple reward shaping...")
        print(f"Max steps: {trade_env.max_step}")
        ...
        for step in range(trade_env.max_step):
            ...
            if action_int > 0 and self.cash[-1] > mid_price:  # Buy
                ...
                self.current_btc += 1
                if step < 10 or step % 200 == 0:  # Log some trades
        ...
        np.save("simple_reward_shaped_evaluation_positions.npy", np.array(positions_cpu))
        np.save("simple_reward_shaped_evaluation_net_assets.npy", np.array(self.net_assets))
        np.save("simple_reward_shaped_evaluation_btc_positions.npy", np.array(self.btc_assets))
        np.save("simple_reward_shaped_evaluation_correct_predictions.npy", np.array(correct_pred))
        ...
        print(f"\n=== SIMPLE REWARD SHAPED EVALUATION RESULTS ===")
...
def run_simple_reward_shaped_evaluation(save_path, agent_list, config_type="balanced"):
    import sys
    ...
    args = Config(agent_class=None, env_class=SimpleRewardShapedEvalTradeSimulator, env_args=env_args)
    args.gpu_id = gpu_id
    args.random_seed = gpu_id
    args.net_dims = (128, 128, 128)
    args.starting_cash = 1e6

    ensemble_evaluator = SimpleRewardShapedEnsembleEvaluator(
        save_path,
        agent_list,
        args,
        reward_config
...
if __name__ == "__main__":
    ...
    print("Testing simple reward shaping configurations...")
    ...
    for config in ["conservative", "balanced", "aggressive"]:
        ...
        print("="*60)
        try:
            run_simple_reward_shaped_evaluation(save_path, agent_list, config)
        except Exception as e:
            print(f"Error with {config} configuration: {e}")
            continue
        print("\n" + "-"*60)